...
public class WitherWaspNestTileEntity extends TileEntity implements ITickableTileEntity {
    private final List<WitherWaspNestTileEntity.Bee> bees = Lists.newArrayList();
    @Nullable
    private BlockPos flowerPos = null;

    public WitherWaspNestTileEntity(final TileEntityType<?> tileEntityTypeIn) {
    ...
    public void angerBees(@Nullable PlayerEntity p_226963_1_, BlockState p_226963_2_, net.minecraft.tileentity.BeehiveTileEntity.State p_226963_3_) {
    ...
    private List<Entity> tryReleaseBee(BlockState p_226965_1_, net.minecraft.tileentity.BeehiveTileEntity.State p_226965_2_) {
    ...
    public void tryEnterHive(Entity p_226962_1_, boolean p_226962_2_, int p_226962_3_) {
        if (this.bees.size() < 3) {
            ...
            this.bees.add(new WitherWaspNestTileEntity.Bee(compoundnbt, p_226962_3_, p_226962_2_ ? 2400 : 600));
            if (this.world != null) {
                if (p_226962_1_ instanceof WaspEntity) {
                    WaspEntity waspentity = (WaspEntity)p_226962_1_;
                    if (waspentity.hasFlower() && (!this.hasFlowerPos() || this.world.rand.nextBoolean())) {
                        this.flowerPos = waspentity.getFlowerPos();
                    }
                }
    ...
    private boolean releaseBee(BlockState p_226967_1_, CompoundNBT p_226967_2_, @Nullable List<Entity> p_226967_3_, net.minecraft.tileentity.BeehiveTileEntity.State p_226967_4_) {
        BlockPos blockpos = this.getPos();
        if ((this.world.isNightTime() || this.world.isRaining()) && p_226967_4_ != net.minecraft.tileentity.BeehiveTileEntity.State.EMERGENCY) {
            return false;
        } else {
            ...
            if (flag && p_226967_4_ != net.minecraft.tileentity.BeehiveTileEntity.State.EMERGENCY) {
                return false;
            } else {
                ...
                if (entity != null) {
                    ...
                    if (!entity.getType().isContained(EntityTypeTags.BEEHIVE_INHABITORS)) {
                        ...
                        if (entity instanceof WaspEntity) {
                            WaspEntity WaspEntity = (WaspEntity)entity;
                            if (this.hasFlowerPos() && !WaspEntity.hasFlower() && this.world.rand.nextFloat() < 0.9F) {
                                WaspEntity.setFlowerPos(this.flowerPos);
                            }

                            if (p_226967_4_ == net.minecraft.tileentity.BeehiveTileEntity.State.HONEY_DELIVERED) {
                                WaspEntity.onHoneyDelivered();
                            ...
                            WaspEntity.resetTicksWithoutNectar();
                            if (p_226967_3_ != null) {
                                p_226967_3_.add(WaspEntity);
                            }
                        }
    ...
    private void tickBees() {
        Iterator<WitherWaspNestTileEntity.Bee> iterator = this.bees.iterator();
        BlockState blockstate = this.getBlockState();

        while(iterator.hasNext()) {
            WitherWaspNestTileEntity.Bee witherwaspnesttileentity$bee = iterator.next();
            if (witherwaspnesttileentity$bee.ticksInHive > witherwaspnesttileentity$bee.minOccupationTicks) {
                ...
                witherwaspnesttileentity$bee.ticksInHive++;
            }
        }
    ...
    public void read(CompoundNBT compound) {
        ...
        for(int i = 0; i < listnbt.size(); ++i) {
            ...
            WitherWaspNestTileEntity.Bee witherwaspnesttileentity$bee = new WitherWaspNestTileEntity.Bee(compoundnbt.getCompound("EntityData"), compoundnbt.getInt("TicksInHive"), compoundnbt.getInt("MinOccupationTicks"));
            this.bees.add(witherwaspnesttileentity$bee);
        }
    ...
    public ListNBT getBees() {
        ...
        for(WitherWaspNestTileEntity.Bee witherwaspnesttileentity$bee : this.bees) {
            ...
            compoundnbt.putInt("MinOccupationTicks", witherwaspnesttileentity$bee.minOccupationTicks);
