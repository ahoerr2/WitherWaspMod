...
//import net.minecraft.tileentity.WitherWaspNestTileEntity;
...
public class WaspEntity extends AnimalEntity implements IFlyingAnimal {
    private static final DataParameter<Byte> DATA_FLAGS_ID = EntityDataManager.createKey(WaspEntity.class, DataSerializers.BYTE);
    private static final DataParameter<Integer> ANGER_TIME = EntityDataManager.createKey(WaspEntity.class, DataSerializers.VARINT);
    ...
    private WaspEntity.PollinateGoal pollinateGoal;
    private WaspEntity.FindBeehiveGoal findBeehiveGoal;
    private WaspEntity.FindFlowerGoal findFlowerGoal;
    private int underWaterTicks;

    public WaspEntity(EntityType<? extends WaspEntity> type, World worldIn) {
        super(type, worldIn);
        this.moveController = new FlyingMovementController(this, 20, true);
        this.lookController = new WaspEntity.BeeLookController(this);
    ...
    protected void registerGoals() {
        ...
        this.pollinateGoal = new WaspEntity.PollinateGoal();
        this.goalSelector.addGoal(4, this.pollinateGoal);
        this.goalSelector.addGoal(5, new FollowParentGoal(this, 1.25D));
        this.goalSelector.addGoal(5, new WaspEntity.UpdateBeehiveGoal());
        this.findBeehiveGoal = new WaspEntity.FindBeehiveGoal();
        this.goalSelector.addGoal(5, this.findBeehiveGoal);
        this.findFlowerGoal = new WaspEntity.FindFlowerGoal();
    ...
    private boolean isHiveNearFire() {
        if (this.hivePos == null) {
            ...
            return tileentity instanceof WitherWaspNestTileEntity && ((WitherWaspNestTileEntity)tileentity).isNearFire();
        }
    }
    ...
    private boolean doesHiveHaveSpace(BlockPos pos) {
        ...
        if (tileentity instanceof WitherWaspNestTileEntity) {
            return !((WitherWaspNestTileEntity)tileentity).isFullOfBees();
        } else {
            return false;
        }
    }
    ...
    private boolean isHiveValid() {
        if (!this.hasHive()) {
            ...
            TileEntity tileentity = this.world.getTileEntity(this.hivePos);
            return tileentity instanceof WitherWaspNestTileEntity;
        }
    }
    ...
    protected PathNavigator createNavigator(World worldIn) {
        FlyingPathNavigator flyingpathnavigator = new FlyingPathNavigator(this, worldIn) {
            ...
            public void tick() {
                if (!WaspEntity.this.pollinateGoal.isRunning()) {
                    super.tick();
                }
            }
        };
    ...
    public WaspEntity createChild(AgeableEntity ageable) {
        return RegistryHandler.WITHER_WASP.get().create(this.world);
    }
    ...
    class AngerGoal extends HurtByTargetGoal {
        AngerGoal(WaspEntity beeIn) {
            super(beeIn);
        }

        protected void setAttackTarget(MobEntity mobIn, LivingEntity targetIn) {
            if (mobIn instanceof WaspEntity && this.goalOwner.canEntityBeSeen(targetIn) && ((WaspEntity)mobIn).setBeeAttacker(targetIn)) {
    ...
    static class AttackPlayerGoal extends NearestAttackableTargetGoal<PlayerEntity> {
        AttackPlayerGoal(WaspEntity beeIn) {
        ...
        private boolean canSting() {
            WaspEntity WaspEntity = (WaspEntity)this.goalOwner;
            return WaspEntity.isAngry() && !WaspEntity.hasStung();
        }
    }
    ...
    class BeeLookController extends LookController {
         ...
         */
        public void tick() {
            if (!WaspEntity.this.isAngry()) {
                super.tick();
            }
        }

        protected boolean func_220680_b() {
            return !WaspEntity.this.pollinateGoal.isRunning();
        }
    }

    class EnterBeehiveGoal extends WaspEntity.PassiveGoal {
        private EnterBeehiveGoal() {
        }

        public boolean canBeeStart() {
            if (WaspEntity.this.hasHive() && WaspEntity.this.canEnterHive() && WaspEntity.this.hivePos.withinDistance(WaspEntity.this.getPositionVec(), 2.0D)) {
                TileEntity tileentity = WaspEntity.this.world.getTileEntity(WaspEntity.this.hivePos);
                if (tileentity instanceof WitherWaspNestTileEntity) {
                    WitherWaspNestTileEntity WitherWaspNestTileEntity = (WitherWaspNestTileEntity)tileentity;
                    if (!WitherWaspNestTileEntity.isFullOfBees()) {
                        return true;
                    }
        ...
        public void startExecuting() {
            ...
            if (tileentity instanceof WitherWaspNestTileEntity) {
                WitherWaspNestTileEntity WitherWaspNestTileEntity = (WitherWaspNestTileEntity)tileentity;
                WitherWaspNestTileEntity.tryEnterHive(WaspEntity.this, WaspEntity.this.hasNectar());
            }
    ...
    public class FindBeehiveGoal extends WaspEntity.PassiveGoal {
        ...
        public boolean canBeeStart() {
            return WaspEntity.this.hivePos != null && !WaspEntity.this.detachHome() && WaspEntity.this.canEnterHive() && !this.isCloseEnough(WaspEntity.this.hivePos) && WaspEntity.this.world.getBlockState(WaspEntity.this.hivePos).isIn(BlockTags.BEEHIVES);
        }
        ...
        public void resetTask() {
            this.ticks = 0;
            WaspEntity.this.navigator.clearPath();
            WaspEntity.this.navigator.resetRangeMultiplier();
        }
         ...
         */
        public void tick() {
            if (WaspEntity.this.hivePos != null) {
                ++this.ticks;
                if (this.ticks > 600) {
                    this.makeChosenHivePossibleHive();
                } else if (!WaspEntity.this.navigator.func_226337_n_()) {
                    if (!WaspEntity.this.isWithinDistance(WaspEntity.this.hivePos, 16)) {
                        if (WaspEntity.this.isTooFar(WaspEntity.this.hivePos)) {
                            this.reset();
                        } else {
                            WaspEntity.this.startMovingTo(WaspEntity.this.hivePos);
                        }
                    } else {
                        boolean flag = this.startMovingToFar(WaspEntity.this.hivePos);
                        if (!flag) {
                            this.makeChosenHivePossibleHive();
                        } else if (this.path != null && WaspEntity.this.navigator.getPath().isSamePath(this.path)) {
                            this.reset();
                        } else {
                            this.path = WaspEntity.this.navigator.getPath();
                        }
        ...
        private void makeChosenHivePossibleHive() {
            if (WaspEntity.this.hivePos != null) {
                this.addPossibleHives(WaspEntity.this.hivePos);
            }
        ...
        private void reset() {
            WaspEntity.this.hivePos = null;
            WaspEntity.this.remainingCooldownBeforeLocatingNewHive = 200;
        }

        private boolean isCloseEnough(BlockPos pos) {
            if (WaspEntity.this.isWithinDistance(pos, 2)) {
                return true;
            } else {
                Path path = WaspEntity.this.navigator.getPath();
                return path != null && path.getTarget().equals(pos) && path.reachesTarget() && path.isFinished();
            }
        }
    }

    public class FindFlowerGoal extends WaspEntity.PassiveGoal {
        private int ticks = WaspEntity.this.world.rand.nextInt(10);
        ...
        public boolean canBeeStart() {
            return WaspEntity.this.savedFlowerPos != null && !WaspEntity.this.detachHome() && this.shouldMoveToFlower() && WaspEntity.this.isFlowers(WaspEntity.this.savedFlowerPos) && !WaspEntity.this.isWithinDistance(WaspEntity.this.savedFlowerPos, 2);
        }
         ...
         */
        public void tick() {
            if (WaspEntity.this.savedFlowerPos != null) {
                ++this.ticks;
                if (this.ticks > 600) {
                    WaspEntity.this.savedFlowerPos = null;
                } else if (!WaspEntity.this.navigator.func_226337_n_()) {
                    if (WaspEntity.this.isTooFar(WaspEntity.this.savedFlowerPos)) {
                        WaspEntity.this.savedFlowerPos = null;
                    } else {
                        WaspEntity.this.startMovingTo(WaspEntity.this.savedFlowerPos);
                    }
                }
            }
        }
        ...
        private boolean shouldMoveToFlower() {
            return WaspEntity.this.ticksWithoutNectarSinceExitingHive > 2400;
        }
    }

    class FindPollinationTargetGoal extends WaspEntity.PassiveGoal {
        private FindPollinationTargetGoal() {
        }

        public boolean canBeeStart() {
            if (WaspEntity.this.getCropsGrownSincePollination() >= 10) {
                return false;
            } else if (WaspEntity.this.rand.nextFloat() < 0.3F) {
                return false;
            } else {
                return WaspEntity.this.hasNectar() && WaspEntity.this.isHiveValid();
            }
        }
         ...
         */
        public void tick() {
            if (WaspEntity.this.rand.nextInt(30) == 0) {
                for(int i = 1; i <= 2; ++i) {
                    BlockPos blockpos = (new BlockPos(WaspEntity.this)).down(i);
                    BlockState blockstate = WaspEntity.this.world.getBlockState(blockpos);
                    ...
                    if (block.isIn(BlockTags.BEE_GROWABLES)) {
                        ...
                        if (flag) {
                            WaspEntity.this.world.playEvent(2005, blockpos, 0);
                            WaspEntity.this.world.setBlockState(blockpos, blockstate.with(integerproperty, Integer.valueOf(blockstate.get(integerproperty) + 1)));
                            WaspEntity.this.addCropCounter();
                        }
                    }
                }
    ...
    abstract class PassiveGoal extends Goal {
         ...
         */
        public boolean shouldExecute() {
            return this.canBeeStart() && !WaspEntity.this.isAngry();
        }
         ...
         */
        public boolean shouldContinueExecuting() {
            return this.canBeeContinue() && !WaspEntity.this.isAngry();
        }
    }

    class PollinateGoal extends WaspEntity.PassiveGoal {
        ...
        public boolean canBeeStart() {
            if (WaspEntity.this.remainingCooldownBeforeLocatingNewFlower > 0) {
                return false;
            } else if (WaspEntity.this.hasNectar()) {
                return false;
            } else if (WaspEntity.this.world.isRaining()) {
                return false;
            } else if (WaspEntity.this.rand.nextFloat() < 0.7F) {
                return false;
            } else {
        ...
        public boolean canBeeContinue() {
            if (!this.running) {
                return false;
            } else if (!WaspEntity.this.hasFlower()) {
                return false;
            } else if (WaspEntity.this.world.isRaining()) {
                return false;
            } else if (this.completedPollination()) {
                return WaspEntity.this.rand.nextFloat() < 0.2F;
            } else if (WaspEntity.this.ticksExisted % 20 == 0 && !WaspEntity.this.isFlowers(WaspEntity.this.savedFlowerPos)) {
                WaspEntity.this.savedFlowerPos = null;
                return false;
            } else {
                return true;
        ...
        public void startExecuting() {
            ...
            this.running = true;
            WaspEntity.this.resetTicksWithoutNectar();
        }
        ...
        public void resetTask() {
            if (this.completedPollination()) {
                WaspEntity.this.setHasNectar(true);
            }

            this.running = false;
            WaspEntity.this.navigator.clearPath();
            WaspEntity.this.remainingCooldownBeforeLocatingNewFlower = 200;
        }
        ...
        public void tick() {
            ...
            if (this.ticks > 600) {
                ...
                if (vec3d.distanceTo(WaspEntity.this.getPositionVec()) > 1.0D) {
                    ...
                    boolean flag = WaspEntity.this.getPositionVec().distanceTo(this.nextTarget) <= 0.1D;
                    boolean flag1 = true;
                    if (!flag && this.ticks > 600) {
                        WaspEntity.this.savedFlowerPos = null;
                    } else {
                        if (flag) {
                            boolean flag2 = WaspEntity.this.rand.nextInt(100) == 0;
                            if (flag2) {
                            ...
                            WaspEntity.this.getLookController().setLookPosition(vec3d.getX(), vec3d.getY(), vec3d.getZ());
                        }
                        ...
                        if (WaspEntity.this.rand.nextFloat() < 0.05F && this.pollinationTicks > this.lastPollinationTick + 60) {
                            this.lastPollinationTick = this.pollinationTicks;
                            WaspEntity.this.playSound(SoundEvents.ENTITY_BEE_POLLINATE, 1.0F, 1.0F);
                        }
        ...
        private float getRandomOffset() {
            return (WaspEntity.this.rand.nextFloat() * 2.0F - 1.0F) * 0.33333334F;
        }
        ...
        private Optional<BlockPos> findFlower(Predicate<BlockState> p_226500_1_, double distance) {
            BlockPos blockpos = new BlockPos(WaspEntity.this);
                ...
                for(int j = 0; (double)j < distance; ++j) {
                    for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                        for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                            ...
                            if (blockpos.withinDistance(blockpos$mutable, distance) && p_226500_1_.test(WaspEntity.this.world.getBlockState(blockpos$mutable))) {
    ...
    class StingGoal extends MeleeAttackGoal {
         ...
         */
        public boolean shouldExecute() {
            return super.shouldExecute() && WaspEntity.this.isAngry() && !WaspEntity.this.hasStung();
        }
         ...
         */
        public boolean shouldContinueExecuting() {
            return super.shouldContinueExecuting() && WaspEntity.this.isAngry() && !WaspEntity.this.hasStung();
        }
    }

    class UpdateBeehiveGoal extends WaspEntity.PassiveGoal {
        private UpdateBeehiveGoal() {
        }

        public boolean canBeeStart() {
            return WaspEntity.this.remainingCooldownBeforeLocatingNewHive == 0 && !WaspEntity.this.hasHive() && WaspEntity.this.canEnterHive();
        }
        ...
        public void startExecuting() {
            ...
            if (!list.isEmpty()) {
                for(BlockPos blockpos : list) {
                    if (!WaspEntity.this.findBeehiveGoal.isPossibleHive(blockpos)) {
                        WaspEntity.this.hivePos = blockpos;
                        return;
                    }
                }

                WaspEntity.this.findBeehiveGoal.clearPossibleHives();
                WaspEntity.this.hivePos = list.get(0);
            }
        }

        private List<BlockPos> getNearbyFreeHives() {
            BlockPos blockpos = new BlockPos(WaspEntity.this);
            PointOfInterestManager pointofinterestmanager = ((ServerWorld)WaspEntity.this.world).getPointOfInterestManager();
            ...
            return stream.map(PointOfInterest::getPos).filter((p_226487_1_) -> {
                return WaspEntity.this.doesHiveHaveSpace(p_226487_1_);
    ...
    class WanderGoal extends Goal {
         ...
         */
        public boolean shouldExecute() {
            return WaspEntity.this.navigator.noPath() && WaspEntity.this.rand.nextInt(10) == 0;
        }
         ...
         */
        public boolean shouldContinueExecuting() {
            return WaspEntity.this.navigator.func_226337_n_();
        }
        ...
        public void startExecuting() {
            ...
            if (vec3d != null) {
                WaspEntity.this.navigator.setPath(WaspEntity.this.navigator.getPathToPos(new BlockPos(vec3d), 1), 1.0D);
            }
        ...
        private Vec3d getRandomLocation() {
            Vec3d vec3d;
            if (WaspEntity.this.isHiveValid() && !WaspEntity.this.isWithinDistance(WaspEntity.this.hivePos, 40)) {
                Vec3d vec3d1 = new Vec3d(WaspEntity.this.hivePos);
                vec3d = vec3d1.subtract(WaspEntity.this.getPositionVec()).normalize();
            } else {
                vec3d = WaspEntity.this.getLook(0.0F);
            }
